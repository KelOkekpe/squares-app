...
const signupHash = "#access_token=eyJhbGciOiJIUzI1NiIsImtpZCI6Imy5ZjJlMTc3&expires_at=1787005687&expires_in=3600&refresh_token=xe7ooioaftubsb&token_type=bearer&type=signup";
...
for (const [label, h] of [["signup", signupHash], ["recovery", recoveryHash], ["error", errorHash]]) {
...
check("error message extracted", authErrorFromHash(errorHash) === "Email link is invalid or has expired");
