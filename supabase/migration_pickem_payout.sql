-- Migration: pick'em winners can say how to be paid
-- Run in the Supabase SQL Editor AFTER migration_drop_phone.sql
--
-- A pick'em winner is owed money exactly as a squares winner is, but the sheet
-- never asked how to send it and pickem_contacts had nowhere to put it. The
-- squares side has had this since migration_entry_privacy.sql; this brings the
-- two into line.
--
-- Same placement as the email: in pickem_contacts, which is admin-only by RLS,
-- and never in the picks blob that every player can read.

ALTER TABLE public.pickem_contacts
  ADD COLUMN IF NOT EXISTS payout_method  TEXT,
  ADD COLUMN IF NOT EXISTS payout_handles JSONB;
...
BEGIN
  ...
  INSERT INTO public.pickem_contacts
    (space_code, pool_id, entry_id, email, payout_method, payout_handles)
...
END;
$$;
-- Signature is unchanged, so the existing grant survives CREATE OR REPLACE.
-- Re-applied from pg_proc anyway rather than typed out — a hand-written
-- signature is what broke the drop-phone migration on its first run.
DO $$
DECLARE fn TEXT;
BEGIN
  FOR fn IN
...
-- Verify:
--   SELECT entry_id, email, payout_method, payout_handles FROM pickem_contacts;
--   -- should be 0: payout details never belong in the readable blob
--   SELECT count(*) FROM spaces s CROSS JOIN LATERAL jsonb_array_elements(s.value) e
--    WHERE s.type = 'picks' AND (e ? 'payoutMethod' OR e ? 'payoutHandles');
