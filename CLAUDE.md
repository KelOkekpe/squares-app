...
The **manual mailto in `ApprovalNotice` is a fallback, not the mechanism** — approval emails the player automatically, and the button appears only when that send actually failed. `sendConfirmationEmail` therefore returns the endpoint's outcome rather than swallowing it; guessing either way leaves the admin lied to or asked to re-send something already sent.
...
**A submitted sheet is emailed to the player** (`api/send-picks.js`), because picks stay hidden until the first kickoff so nobody can copy them — which otherwise leaves the player with no record of what they chose. The endpoint is unauthenticated, since players are anonymous, so three things bound it: the recipient is read from `pickem_contacts` against the entry id and never taken from the request, only entries submitted in the last hour are eligible, and entry ids are UUIDs. It needs `RESEND_API_KEY` and `PICKS_FROM_EMAIL`; without them it is a no-op rather than an error, because the picks are already saved by the time it runs. The client never awaits it for the same reason.

**Pick'em is gated too, but differently.** The sheet is recorded immediately — picks have to be locked in before kickoff, and holding them in a queue until an admin looked would lose them. What waits is whether the sheet **counts**: `Standings` ranks only `paid` entries when `config.entryFee > 0`. It says so **only about the reader's own sheet**, matched via `myEntryId(poolId)` from the local store — a running count of who else hasn't paid is the admin's business, and showing it to everyone put other people's payment status in front of the whole pool. A contest with no fee marks entries paid on arrival, decided from the board's own config inside `submit_picks` — reading the fee from the request would let a client declare itself free. `rankEntries` stays payment-blind; the filter belongs to the view, not the rules of the game. The trust boundary is a human, not the database.
